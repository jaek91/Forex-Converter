...
class FlaskTests(TestCase):
    ...
    ##--todo: Ask about this test next time--##
...
class FormFuncTests(TestCase):
    ...
    def test_validate_currency(self):
        """This is a test whether we have valid currencies to convert"""

        ## test validate currency from ##
        self.assertEqual(validate_currency("USD"), True) ## we expect both to be True because USD is supported
        self.assertFalse(validate_currency("BDT")) ## since BDT is not supported, validate should be false

        ##test validate currency to ##
        self.assertEqual(validate_currency("USD"), True)
        self.assertFalse(validate_currency("BDT"))
    ...
    def test_calc_converted_currency(self):
        ...
        self.assertEqual(calc_converted_currency("USD","USD", 1), f"The result is {dollar_symbol} 1.00")
        self.assertEqual(calc_converted_currency("EUR","EUR", 1), f"The result is {euro_symbol} 1.00")